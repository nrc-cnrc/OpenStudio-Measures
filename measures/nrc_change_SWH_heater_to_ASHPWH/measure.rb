...
class NrcChangeSWHtoASHPWH < OpenStudio::Measure::ModelMeasure
  ...
  def run(model, runner, user_arguments)
    ...
    model.getPlantLoops.each do |plantloop|
	  ...
	  if plantloop.name.to_s.include?("Service Water") 
		...
		puts "Capacity #{heater_capacity_W}".red
		puts "Volume #{heater_volume_m3}".red
		puts "Schedule #{setpoint_schedule}".red
  ...
  # Adds heater to the supplied ploant loop and condenser to the thermal zone.
  ...
  def add_ashpwh(model, runner, heater_tank, frac_oa)
    ...
    model.getZoneHVACEquipmentLists.each do |zoneHVACEquipmentList|
	  ...
	  if coolingLoad.is_initialized then # Not sure why we need to use the is_initialised method here. Without it the optional does not work as expected.
	  ...
	  # Selecte the largest zone as the default zone for the compressor (if a coolZone is not found)
	...
	if coolZone then
	  ...
	  puts "Adding compressor to the largest zone".red
