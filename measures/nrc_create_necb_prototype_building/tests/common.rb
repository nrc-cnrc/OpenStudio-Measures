...
module TestCommon
  ...
  class NrcCreateNECBPrototypeBuilding_Test < Minitest::Test
    ...
    def run_test(necb_template:, building_type_in:, fuel_in: 'DefaultFuel', location_in:, weather_file_type_in:, global_warming_in:)
      puts "Testing  model creation for ".green + "#{building_type_in}-#{necb_template}-#{location_in}-#{fuel_in}".light_blue
      ...
      puts "  Water Heating fuel: ".green + " #{fuel_in}".light_blue
      puts "  Heating fuel: ".green + " #{fuel_in}".light_blue

      # Make an empty model.
      model = OpenStudio::Model::Model.new

      input_arguments = {
        "necb_template" => necb_template,
      ...
      model_name = "#{necb_template}-#{building_type_in}-#{location_in}_#{weather_file_type_in}_#{global_warming_in.to_i}_#{fuel_in}"
      ...
      # Check for no errors.
      assert_equal(0, diffs.size, 'Differences detected for model #{model_name}')
    end
  end
end
